...
### Модальні вікна — ОБОВʼЯЗКОВО блокувати фоновий скрол

**Правило:** будь-яке модальне вікно або повноекранний overlay (Родовід, Кабінет,
Бібліотека, Розстановка, OnboardingFlow тощо) ПОВИНЕН блокувати скрол сторінки
позаду. Інакше користувач бачить два скроли (внутрішній модалки + зовнішній
сторінки) — це баг.

**Як правильно:** використовуй хук `useOverlayA11y(onClose)` з
`src/hooks/useOverlayA11y.js`. Він уже:
- блокує `<html>` і `<body>` одночасно (важливо: у різних браузерах
  scroll-target різний — у Chrome це `<html>`, у Safari — `<body>`),
- ставить `position: fixed; top: -scrollY` на body — зберігає scroll position
  щоб після закриття сторінка не стрибнула вгору,
- підтримує стек overlay (вкладені модалки розблокують body тільки коли
  закриється останній),
- ESC закриває top-most overlay.

**Не дублюй ручний `body.style.overflow = 'hidden'` всередині компонентів** —
один body-state на застосунок, керується тільки через `pushOverlay`/`popOverlay`
у хуці.

**Сам overlay-контейнер** — `position: fixed; inset: 0; overflow-y: auto` для
внутрішнього скролу довгого вмісту. Додай `overscroll-behavior: contain` щоб
тач/колесо не "перетікало" з overlay на сторінку коли впираєшся в край.
